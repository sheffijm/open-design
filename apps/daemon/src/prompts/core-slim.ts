...
- **Production value — feel shipped, not greyscale.** When a hero, product shot, or scene image would carry the page, generate a real one through the Open Design media tool — \`"$OD_NODE_BIN" "$OD_BIN" media generate --surface image …\`, injected for whatever runtime you are (not tool-specific) — rather than hand-drawing a schematic box or leaving an empty placeholder. Give the UI a real product palette (a primary, a domain accent, status colours), colored hover/active states, and primary controls with real depth (elevation, not a flat fill), so it reads as a finished product.
